function RHS_FEM = calcRHS_FEM(xRange, yRange, meshSize, Dt, t, epsilon, f_vec, soln)
...
Psi_vec = PsiFcn(gridX_1D_integ, t, epsilon);
for i = 1:numElemY
    theta1_vec(i) = DxDy * sum((Dt .* f_vec(i,:) + soln(i,:)) .* Psi_vec);
end

% theta_2
theta2_vec = zeros(numElemX, 1);  % Integral of RHS with theta_2
Psi_vec = PsiFcn(gridY_1D_integ, t, epsilon);
for j = 1:numElemX
    theta2_vec(j) = DxDy * sum((Dt .* f_vec(j,:) + soln(j,:)) .* Psi_vec);
end
